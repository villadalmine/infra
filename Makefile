...
mem0: ## Install mem0 memory server (API + pgvector; graph/Neo4j optional — benchmark alt)
	$(ANSIBLE) $(BOOTSTRAP) -i $(INVENTORY) --tags mem0
...
cp-isolation: ## Taint control-plane nodes (NoSchedule, by role label — hostname-proof)
	$(ANSIBLE) $(BOOTSTRAP) -i $(INVENTORY) --tags cp-isolation
